    ...
    fluidRow(
      ...
      shinydashboardPlus::box(
        ...
        column(
          ...
          p(
            "Appropriate study design is dependent on",
            "the type of", span("tracking device", class = "cl-sea-d"),
            "(and, ultimately, its",
            wrap_none(span("battery life", class = "cl-sea-d"), ")"),
      ...
      div(
        ...
        div(
          class = "col-lg-6 no-padding-left",
          shinydashboardPlus::box(
        ...
        div(
          class = "col-lg-6 no-padding-right",
          shinydashboardPlus::box(
...
mod_tab_device_server <- function(id, vals) {
  moduleServer(id, function(input, output, session) {
    ...
    output$min_frq <- renderText({
      ...
      out <- "1 fix every"
      out <- dplyr::case_when(
        input$gps_dti_max == paste(out, "day") ~ "= 1 fix",
        input$gps_dti_max == paste(out, "12 hours") ~ "= 2 fixes",
        input$gps_dti_max == paste(out, "8 hours") ~ "= 3 fixes",
        input$gps_dti_max == paste(out, "6 hours") ~ "= 4 fixes",
        input$gps_dti_max == paste(out, "4 hours") ~ "= 6 fixes",
        input$gps_dti_max == paste(out, "2 hours") ~ "= 12 fixes",
        input$gps_dti_max == paste(out, "hour") ~ "= 24 fixes",
        input$gps_dti_max == paste(out, "2 minutes") ~ "= 720 fixes",
        input$gps_dti_max == paste(out, "minute") ~ "= 1440 fixes")
      out <- paste(out, "every day")
      return(out)
        ...
        switch(input$device_type,
          ...
          GPS = {
            column(
              ...
              p(
                "What", span("sampling interval", class = "col-hgl"),
                "will you evaluate?"
              ) %>%
    ...
    observe({
      ...
      if (length(vals$which_question) > 1) {
        ui <- tagList(
          "Note: The secondary axis is showing rough",
          "estimates of the effective sample sizes",
        ...
        switch(vals$which_question,
          "Home range" = {
            ui <- tagList(
              ...
              "rough estimates of the effective sample size",
              span("N[area]", class = "cl-sea"),
              "for each sampling design; true N values",
              "may differ."
            )
          },
          "Speed & distance" = {
            ui <- tagList(
              "Note: The secondary axis is showing",
              "rough estimates the effective sample size",
      ...
      output$regPlotSubtitle <- renderUI({
        ui
      })
    }) %>% # end of observe
    ...
    observe({
      req(!is.null(input$eval_tradeoffs), vals$data0)
      ...
      if ((!input$eval_tradeoffs && is.null(input$gps_dti)) ||
        ...
        validate(
          need(
            vals$reg$dur != "" && vals$reg$dur > 0,
            "Requires a non-zero value."
          ),
          need(
            vals$reg$dti != "" && vals$reg$dti > 0,
            "Requires a non-zero value."
          )
        )
        ...
        if (vals$device_n > vals$storage) {
          shinyalert::shinyalert(
            ...
            text = span(
              "GPS storage limit",
              HTML(paste0(
                "(", scales::label_comma()(vals$storage),
                " locations)"
              )),
              "is lower than the absolute sample size",
              HTML(paste0(
                "(", scales::label_comma()(vals$device_n),
                " locations)."
              ))
            ),
            html = TRUE,
    ...
    observe({
      ...
      if (input$device_type == "GPS") {
        ...
        if (!is.null(input$regPlot_vhf_selected)) {
          ...
          validate(
            need(input$vhf_dur != "", "Requires a value."),
    ...
    gps_simulation <- shiny::reactive({
      ...
      dat <- simulate_gps(
        ...
        b_max = input$gps_dur,
        b_unit = input$gps_dur_units,
        dti_max = input$gps_dti_max,
        dti_min = input_cutoff)

      if (all(dat$dur_sec == 0)) {
        return(NULL)
      }
    ...
    observe({
      ...
      if (!is.null(input$deviceInput_error)) {
        if (input$deviceInput_error > 0) {
          error_x <- stats::rnorm(nrow(data1),
            mean = 0,
            sd = input$deviceInput_error
          )
          error_y <- stats::rnorm(nrow(data1),
            mean = 0,
            sd = input$deviceInput_error
          )
    ...
    reg_selected <- reactive({
      if (input$device_type == "GPS") {
        return(input$regPlot_gps_selected)
      }
      if (input$device_type == "VHF") {
        return(input$regPlot_vhf_selected)
      }
    })
    ...
    observe({
      ...
      vals$reg$add_N1 <- vals$reg$add_N2 <- TRUE
      if ("Home range" %in% vals$which_question) {
        if (!is.null(input$show_effective)) {
          vals$reg$add_N1 <- ifelse(
            "n_area" %in% input$show_effective, TRUE, FALSE
          )
        }
      }
      ...
      if ("Speed & distance" %in% vals$which_question) {
        if (!is.null(input$show_effective)) {
          vals$reg$add_N2 <- ifelse(
            "n_speed" %in% input$show_effective, TRUE, FALSE
          )
        }
      }
    })
    ...
    observe({
      ...
      if ((input$gps_dur %#% input$gps_dur_units <= 2 %#% "days") ||
        (input$gps_dur %#% input$gps_dur_units > 20 %#% "years")) {
        proceed <- NULL

        if (vals$keep_alert && !vals$tour_active) {
          shinyalert::shinyalert(
            type = "warning",
            title = "Warning",
            text = tagList(
      ...
      if (!("black" %in% gps_sim$color)) {
        pal_values <- pal$dgr
      } else {
        pal_values <- c(pal$mdn, pal$dgr)
      }
      ...
      output$regPlot_gps <- ggiraph::renderGirafe({
        ...
        p <- ggplot2::ggplot(
          ...
          {
            if (min(gps_sim$frq_hrs) <= 0.04166667) {
              ggplot2::geom_vline(
                ggplot2::aes(
                  xintercept = 0.04166667,
                  linetype = "1 day"
                )
              )
            }
            ...
            if (!is.null(input$deviceInput_log)) {
              if (input$deviceInput_log) {
                ggplot2::scale_x_log10(
                  "Log10 of frequency (fixes per hour)"
                )
              }
            }
          } +
          ...
          ggplot2::theme(legend.position = c(0.65, 0.85))
