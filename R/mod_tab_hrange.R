          ...
          tabsetPanel(
            ...
            tabPanel(
              ...
              title = tagList(
                ...
                span("Simulation", class = "ttl-panel")
              ),
              div(
                ...
                column(
                  ...
                  style = "z-index: 1;",
                  shiny::checkboxInput(
                    inputId = ns("show_truth"),
                    label = "Show the true 95% area",
                    value = TRUE
                  )
                ),
            ...
            tabPanel(
              ...
              div(
                ...
                column(
                  ...
                  style = "z-index: 1;",
                  shiny::checkboxInput(
                    inputId = ns("hrShow_initial"),
                    label = span(
                      "Add the initial",
                      icon("stopwatch", class = "cl-sea"),
                      span("Simulation", class = "cl-sea"),
                      "locations to the plot below"
                    ),
                    value = FALSE
                  ),
                  shiny::checkboxInput(
                    inputId = ns("hrShow_modified"),
                    label = span(
                      "Add the modified",
                      icon("bolt", class = "cl-mdn"),
                      span("Simulation", class = "cl-mdn"),
                      "locations to the plot below"
                    ),
                    value = TRUE
                  )
...
mod_tab_hrange_server <- function(id, vals) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      if (!is.null(vals$akde)) {
        ...
        if (vals$data_type == "simulated") {
          ...
          tmprange <-
            paste(
              ifelse(vals$tau_p0$value[1] == 0, "0",
                scales::label_comma(
                  accuracy = .1
                )(vals$tau_p0$value[1])
              ),
        ...
        shiny::showModal(
          shiny::modalDialog(
            ...
            fluidRow(
              ...
              p(
                "If home range estimation is your goal,",
    ...
    output$hrInfo_est <- shiny::renderUI({
      ...
      est_min <- fix_unit(
        est$unit %#% vals$hrEst[[1]] %#%
          vals$hr_units,
        est$unit,
        ui = TRUE
      )
      est_max <- fix_unit(
        est$unit %#% vals$hrEst[[2]] %#%
          vals$hr_units,
        est$unit,
        ui = TRUE
      )
    ...
    output$hrPlot_initial <- ggiraph::renderGirafe({
      ...
      sigma0 <- vals$sigma0$value[2] %#% vals$sigma0$unit[2]
      ud <- plotting_hr(
        data = vals$data1,
        sigma = sigma0,
        ud = vals$akde,
        opts = input$hrShow_levels,
        show_truth = input$show_truth
      )
    ...
    output$hrPlot_modified <- ggiraph::renderGirafe({
      ...
      sigma0 <- vals$sigma0$value[2] %#% vals$sigma0$unit[2]
      ud_new <- plotting_hr(
        initial_data = vals$data1,
        data = vals$hr$newdata,
        sigma = sigma0,
        ud = vals$akde_new,
        opts = input$hrShow_levels_new,
        show_initial_data = input$hrShow_initial,
        show_data = input$hrShow_modified,
        show_truth = input$show_truth
      )
    ...
    observe({
      shiny::showModal(
        shiny::modalDialog(
          ...
          p(
            "The", span("relative error (%)", class = "cl-dgr"),
