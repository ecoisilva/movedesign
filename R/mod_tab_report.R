...
mod_tab_report_ui <- function(id) {
  ...
  tagList(
    fluidRow(
      ...
      div(
        class = "col-xs-12 col-sm-8 col-md-8 col-lg-10",

        ## Parameter information: ---------------------------------------

        div(
          class = "col-lg-6 no-padding-left tabBox_noheaders",
          shinydashboardPlus::box(
            id = ns("repBox_pars"),
            width = NULL,
            headerBorder = FALSE,
        ...
        div(
          class = "col-lg-6 no-padding-right tabBox_noheaders",
          shinydashboardPlus::box(
      ...
      div(
        ...
        shinydashboardPlus::box(
          ...
          div(
            ...
            div(
              id = ns("section-two_questions"),
              uiOutput(ns("end_report_both"))
            ),
            div(
              id = ns("section-comparison"),
              p(
                "Quick comparison with other",
...
mod_tab_report_server <- function(id, vals) {
  moduleServer(id, function(input, output, session) {
    ...
    output$repBlock_speed <- shiny::renderUI({
      req(vals$fit0)
    ...
    output$plot_hr_density <- ggiraph::renderGirafe({
      req(vals$which_question, vals$ci)
      ...
      if (is_dur) {
        ...
        vals$hrCI_new <-
          suppressWarnings(bayestestR::ci(
            ds2_hr$x,
            ci = input_ci, method = "HDI"
          ))

        if (is_log) ds2_hr$y <- ds2_hr$y / max(ds2_hr$y)

        ci2_hr <- subset(
          ds2_hr, x >= vals$hrCI_new$CI_low & x <= vals$hrCI_new$CI_high
        )
      ...
      bindEvent(list(
        input$create_report,
        input$repInput_scaled,
        input$highlight_dur
      ))

    observe({
      req(
        vals$data0, vals$tau_v0,
        vals$reg$dur, vals$reg$dur_unit,
      ...
      if (is_dti) {
        ...
        dti_new <- sd_opts$dti[match(
          input$highlight_dti,
          sd_opts$dti_notes
        )]
        out_dti_new <- dt_sd$dti[which.min(abs(dt_sd$dti - dti_new))]
        txt_dti_new <- sd_opts$dti_notes[match(
          out_dti_new,
          sd_opts$dti
        )]
        ...
        ci2_sd <- subset(
          ds2_sd, x >= vals$sdCI_new$CI_low & x <= vals$sdCI_new$CI_high
        )
    ...
    ### Rendering density plot details: -----------------------------------

    output$plot_density_details <- ggiraph::renderGirafe({
      req(vals$hrCI, vals$sdCI)
      ...
      if (is_dur && !is_both) {
        ...
        details <- details %>%
          dplyr::add_row(
            ...
            lci = vals$hrCI_new$CI_low,
            uci = vals$hrCI_new$CI_high,
            label = paste0(
              "Median AKDE error + ",
              vals$ci, "% HDI for ",
      ...
      if (is_dti && !is_both) {
        details <- details %>%
          dplyr::add_row(
            ...
            lci = vals$sdCI_new$CI_low,
            uci = vals$sdCI_new$CI_high,
            label = paste0(
              "Median CTSD error + ",
              vals$ci, "% HDI for ",
      ...
      if ("Home range" %in% vals$which_question) {
        ...
        details <- details %>%
          dplyr::add_row(
            ...
            lci = vals$hrCI$CI_low,
            uci = vals$hrCI$CI_high,
            label = paste0(
              "Median AKDE error + ",
              vals$ci, "% HDI for ",
        ...
        details <- details %>%
          dplyr::add_row(
            ...
            type = "hr_now",
            value = ifelse(
              !is.null(vals$hrErr), vals$hrErr[[2]], NA
            ),
            lci = ifelse(
              !is.null(vals$hrErr), vals$hrErr[[1]], NA
            ),
            uci = ifelse(
              !is.null(vals$hrErr), vals$hrErr[[3]], NA
            ),
            label = "Current AKDE error",
            fill = pal$sea_d,
            col = pal$sea_d,
            linetype = "dashed",
            shape = 19
      ...
      if ("Speed & distance" %in% vals$which_question) {
        ...
        details <- details %>%
          dplyr::add_row(
            ...
            lci = vals$sdCI$CI_low,
            uci = vals$sdCI$CI_high,
            label = paste0(
              "Median CTSD error + ",
              vals$ci, "% HDI for ",
              vals$report$txt_dti
        ...
        details <- details %>%
          dplyr::add_row(
            ...
            type = "sd_now",
            value = ifelse(
              !is.null(vals$speedErr), vals$speedErr[[2]], NA
            ),
            lci = ifelse(
              !is.null(vals$speedErr), vals$speedErr[[1]], NA
            ),
            uci = ifelse(
              !is.null(vals$speedErr), vals$speedErr[[3]], NA
            ),
            label = "Current CTSD error",
      ...
      if (is_both) {
        add_val <- ifelse(is.null(vals$speedErr), 1, 0)
      ...
      p <- ggplot2::ggplot() +
        # { if (is_both)
        #   ggplot2::facet_wrap(~ .data$question, ncol = 2,
        #                       scale = "free") } +
    ...
    observe({
      ...
      questions <- NULL
      if ("Home range" %in% vals$which_question) {
        req(vals$hr$completed)
        questions <- "Home range"
      }

      if ("Speed & distance" %in% vals$which_question) {
        req(vals$sd$completed)
      ...
      if ("Home range" %in% vals$which_question) {
        ...
        out_regime <- out_reg_hr <-
          p(
            "The ideal", span("sampling duration", class = "cl-sea"),
      ...
      if ("Speed & distance" %in% vals$which_question) {
        ...
        out_regime <- out_reg_ctsd <-
          p(
            "The minimum", span("sampling interval", class = "cl-sea"),
      ...
      if ("Speed & distance" %in% vals$which_question) {
        ...
        if (vals$is_ctsd) {
          out_ctsd2 <-
            span(
              "Keep in mind that, for a similar interval of",
              wrap_none(dti$value, " ", dti$unit, ","),
              "there is a",
    ...
    observe({
      ...
      out_analyses <- p(
        ...
        if (vals$device_N2 > 0) {
          span(
            "For speed estimation, your error estimate",
            "based on a single simulation was",
    ...
    observe({
      ...
      is_both <- ifelse(length(vals$which_question) > 1,
        "Yes", "No"
      )
          ...
          output$end_report_both <- renderUI({
            ...
            out <- tagList(
              splitLayout(
                cellWidths = c("50%", "50%"),
                tagList(
                  p(span("Home range", class = "cl-mdn")) %>%
                    tagAppendAttributes(class = "subheader"),
                  ggiraph::girafeOutput(
                    outputId = ns("plot_hr_density"),
                    width = "100%", height = "100%"
                ...
                tagList(
                  p(span("Speed & distance", class = "cl-mdn")) %>%
                    tagAppendAttributes(class = "subheader"),
                  ggiraph::girafeOutput(
                    outputId = ns("plot_sd_density"),
                    width = "100%", height = "100%"
    ...
    #   ) # end of tagList
    # }) %>% # end of renderUI // end_report
    #   bindEvent(list(vals$which_question,
    #                  input$create_report))
    #
    # output$end_report_both <- renderUI({
    #   req(length(vals$which_question) > 1)
    ...
    observe({
      ...
      if (length(vals$which_question) == 1 &
        ...
        if (highlighted_dur >= ideal_dur$value) {
          ...
          out_comp <- out_comp_hr <-
            p(
              "Your new sampling duration may be insufficient",
              "for", span("home range", class = "cl-grn"),
              "estimation.", br(),
      ...
      if (length(vals$which_question) == 1 &
        ...
        highlighted_dti <- opts$dti[match(
          input$highlight_dti,
          opts$dti_notes
        )]
        ...
        if (highlighted_dti <= ideal_dti$value) {
          ...
          out_comp <- out_comp_sd <-
            p(
              "Your new sampling interval may be insufficient",
              "for", span("speed & distance", class = "cl-grn"),
