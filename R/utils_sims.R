...
#' user-defined probability (`fail_prob`) of failing. If a failure
#' occurs, the dataset is truncated after a random time point.
#' 
...
#'   
#' @param burn_in An integer (default = `10`), the number of initial
#'   locations protected from truncation. Failure can only occur after
#'   this many fixes, so any retained dataset keeps at least `burn_in`
#'   locations. For datasets with `burn_in` or fewer locations, the
#'   protected window shrinks to `nrow(x) - 1`, leaving the final
#'   location eligible for truncation.
#' 
#' @return
...
#'
#' @noRd
.trigger_disruption <- function(obj,
                                fail_prob = 0,
                                burn_in = 10) {
  
  if (is.null(fail_prob) || fail_prob == 0)
    return(list(obj = obj, dev_failed = logical(length(obj))))
  ...
  out <- lapply(obj, function(x) {
    ...
    if (failure_occurred) {
      ...
      n_protect <- max(min(burn_in, nrow(x) - 1), 0)
      to_keep_vec <- c(rep(1, n_protect), cumprod(
        1 - stats::rbinom(nrow(x) - n_protect, 1, prob = 0.01)))
      if (!any(to_keep_vec == 0)) failure_occurred <- FALSE
      x <- x[to_keep_vec == 1, ]
    }
    
    return(list(data = x, failed = failure_occurred))
    
  })
  
  return(list(obj = lapply(out, `[[`, "data"),
              dev_failed = vapply(out, `[[`, 
                                  logical(1), "failed")))
