      ...
      div(
        ...
        shinydashboardPlus::box(
          ...
          column(
            ...
            p(
              "Choose parameters that reflect your intended",
      ...
      div(
        ...
        shinydashboardPlus::box(
          ...
          shiny::selectizeInput(
            inputId = ns("area0_contour"),
            label = "Home range area:",
            choices = c(
              "95% contour" = .95,
              "50% contour" = .50
            ),
...
mod_tab_sims_server <- function(id, vals) {
  moduleServer(id, function(input, output, session) {
    ...
    output$sims_speed <- renderText({
      ...
      if (vals$sigma0$value[2] == "" ||
        input$tau_p0 == "" ||
        input$tau_v0 == "") {
        v <- data.frame(value = "", unit = "")
      } else {
        v <- sqrt((vals$sigma0$value[2] %#% vals$sigma0$unit[2]) * pi / 2) /
    ...
    observe({
      req(input$area0_units != vals$area0$unit)

      new_area0 <- sigdigits(
        input$area0_units %#%
          vals$area0$value %#% vals$area0$unit, 3
      )
      ...
      updateNumericInput(
        ...
        inputId = "area0",
        label = NULL,
        min = 1, value = new_area0
      )
    }) %>% bindEvent(list(
      input$area0_contour,
      input$area0, input$area0_units
    ))
    ...
    observe({
      ...
      vals$area0 <- data.frame(
        value = input$area0,
        unit = input$area0_units
      )

      area <- vals$area0$value %#% vals$area0$unit
      level <- as.numeric(input$area0_contour)
      sigma <- area / (-2 * log(1 - level) * pi)
      sigma <- fix_unit(sigma, "m^2", convert = TRUE)

      vals$sigma0 <- data.frame(
        value = c(NA, sigma$value, NA),
        unit = rep(sigma$unit, 3)
      )
      vals$is_run <- FALSE
    ...
    observe({
      ...
      if (!is.null(vals$seed0)) {
        validate(
          ...
          need(input$area0 != "", "Select a value."),
          need(input$area0_units != "", "Please choose a unit.")
        )
        ...
        shinybusy::show_modal_spinner(
          ...
          text = span(
            ...
            HTML(paste0(
              span("movement data", class = "cl-sea"),
    ...
    output$console_sims <- renderText({
      req(vals$time_sims)
      paste0(
        "The simulation took approximately ",
        round(vals$time_sims, 1), " minutes."
      )
    })
  }) # end of moduleServer
